<template lang='html'>
    ...
    <div>
      <div class="guide">
        <a href="https://linderoths.se/">
          <img alt="Linderoths" width="120.99" height="19.787" style="margin-bottom: 18px;" src="https://linderoths.se/wp-content/uploads/2016/12/linderoths_logo.png" />
        </a>
      </div>
      <div style=" margin-bottom: 2em ;">
        <p class="guide" style="font-family: 'Andale Mono', monospace; font-size: 11px;" v-if="name || titel || businessArea">
          <span class="marginToInfo" style="color: #01215a;">{{ name }}</span>
          <span class="marginToInfo" style="color: #FC5961;">•</span>
          <span class="marginToInfo" style="color: #01215a;">{{ titel }},</span>
          <span style="color: #01215a;">{{ businessArea }}</span>
        </p>
        <div class="guide" style="font-family: 'Andale Mono', monospace; font-size: 11px; margin-bottom:1px;">
          <span class="marginToInfo" style="color: #FC5961;" v-if="phone">•</span>
          <a class="marginToInfo" v-if="phone" style="color: #01215a;text-decoration: none;" :href="`tel:${phoneFormat}`">{{ phone }}</a>
        <span class="marginToInfo" v-if="homepage" style="color: #FC5961;"> • </span>
        <a class="marginToInfo" v-if="homepage" style="color: #FC5961;"  :href="`https://${hemsidan}`">{{ homepage }}</a>
      <span class="marginToInfo" v-if="social" style="color: #FC5961;"> • </span>
      <span class="marginToInfo" v-if="social" style="color: #01215a;">{{ social }}</span>
    </div>
    <div class="guide" style="font-family: 'Andale Mono', monospace; font-size: 11px;">
      <span class="marginToInfo" v-if="local" style="color: #FC5961;"> • </span>
      <span class="marginToInfo" v-if="local" style="color: #01215a;" >{{ local }}</span>
      <span class="marginToInfo" v-if="street" style="color: #FC5961;">•</span>
      <span class="marginToInfo" v-if="street" style="color: #01215a;" >{{ street }}</span>
      <span class="marginToInfo" v-if="postNumber" style="color: #FC5961;">•</span>
      <span class="marginToInfo" v-if="postNumber" style="color: #01215a;">{{ postNumber }}</span>
  ...
  <div class="form">
    ...
    <div class="form-group">
      <div class="label guideFont">
        <label for="formGroupExampleInput">Social Media:</label>
      </div>
      <input v-model="social" type="text" class="form-control" id="formGroupExampleInput" v-on:keyup="setSocial" placeholder="@wearelinderoths">
      </div>
    <div class="form-group">
      <div class="label guideFont">
  ...
  <div class="card guideFont guidecard">
    ...
    <ul class="list-group list-group-flush">
      <li class="list-group-item">Use Chrome</li>
      <li class="list-group-item">Fill in the form</li>
      <li class="list-group-item">Text select the signature</li>
      <li class="list-group-item">Copy! Ctrl+C (pc) or CMD+C (mac)</li>
      <li class="list-group-item">Open Mail app, goto settings, then mail signature settings</li>
      <li class="list-group-item">Paste, save...</li>
    </ul>
  </div>
  </div >
</template >
...
export default {
  ...
  methods: {
    // buttonClick (ev) {
    //   // this.$refs.info.select()
    //   // document.execCommand('copy')
    // },
    setName () {
      localStorage.setItem('mail-signature_name', this.name)
    },
    setTitel () {
      localStorage.setItem('mail-signature_titel', this.titel)
    },
    setBusinessArea () {
    ...
    setSocial () {
      localStorage.setItem('mail-signature_social', this.social)
    },
    setLocal () {
      localStorage.setItem('mail-signature_local', this.local)
    },
    setStreet () {
      localStorage.setItem('mail-signature_street', this.street)
    },
    setPostNumber () {
      localStorage.setItem('mail-signature_post', this.postNumber)
    },
    setCity () {
      localStorage.setItem('mail-signature_city', this.city)
    }
  }
}
</script>
